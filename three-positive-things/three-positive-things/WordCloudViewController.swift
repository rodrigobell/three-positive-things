...
class WordCloudViewController: UIViewController {
  ...
  override func viewDidAppear(_ animated: Bool) {
    Analytics.logEvent("word_cloud_appeared", parameters: nil)
    let daysLogged = getNumberOfDaysLogged()
    self.daysLoggedLabel.text = String(daysLogged) + " days logged"
  }
  ...
  func updateSphereView() {
    ...
    for wordTouple in words {
      ...
      btn.addTarget(self, action: #selector(self.wordCloudButtonTapped(_:)), for: .touchUpInside);
  ...
  @objc func wordCloudButtonTapped(_ sender : UIButton) {
    Analytics.logEvent("word_cloud_btn_tapped", parameters: nil)
  }
  
  @IBAction func segmentedControlDaysBackIndexChanged(_ sender: Any) {
    Analytics.logEvent("toggle_days_back_segmented_control", parameters: nil)
  ...
  @IBAction func segmentedControlTopWordsIndexChanged(_ sender: Any) {
    Analytics.logEvent("toggle_top_words_segmented_control", parameters: nil)
